...
function validateRequiredImageSlots(template, imageRequirements = {}) {
  const requiredSlots = template.imageSlots || [];
  for (const slot of requiredSlots) {
    if (!(slot in imageRequirements)) {
      return { ok: false, code: '422_REQUIRED_IMAGE_SLOT_MISSING' };
    }
  }
  return { ok: true };
}
...
function validateSlide(workspace, slide) {
  ...
  const imageRequired = validateRequiredImageSlots(template, slide.imageRequirements);
  if (!imageRequired.ok) return imageRequired;
  ...
  for (const [slot, requirement] of Object.entries(slide.imageRequirements || {})) {
    if (!template.imageSlots.includes(slot)) {
      return { ok: false, code: '422_SLOT_ASSIGNMENT_INVALID' };
    }

    if (!requirement || typeof requirement !== 'object' || Array.isArray(requirement)) {
      return { ok: false, code: '422_IMAGE_REQUIREMENT_INVALID' };
    }
