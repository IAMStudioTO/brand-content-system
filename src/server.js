...
const { planSlides } = require('./services/ai-planner');
...
async function handler(req, res) {
  ...
  if (req.method === 'POST' && path === '/api/v1/client/content/generate') {
    try {
      ...
      const generated = planSlides(workspace, body.prompt);
      const validation = validateGeneratedContent(workspace, generated);
      if (!validation.ok) {
        return sendJson(res, 422, { error: validation.code });
      }

      const contentId = generateId('cnt');
      state.contents.set(contentId, generated);

      return sendJson(res, 200, { contentId, ...generated });
    } catch (error) {
  ...
  if (req.method === 'GET' && previewMatch) {
    const content = state.contents.get(previewMatch[1]);
    if (!content) return sendJson(res, 404, { error: 'Not found' });
    return sendJson(res, 200, toRenderPayload(workspace, content));
  }

  const exportMatch = path.match(/^\/api\/v1\/client\/content\/([^/]+)\/export$/);
  if (req.method === 'POST' && exportMatch) {
    const content = state.contents.get(exportMatch[1]);
    if (!content) return sendJson(res, 404, { error: 'Not found' });
