...
test('validateSlide fails when required image slot is missing', () => {
  const slide = {
    role: 'problem',
    templateId: 'template_explainer_01',
    textAssignments: {
      headline: 'Titolo',
  ...
  const result = validateSlide(workspace, slide);
  assert.equal(result.ok, false);
  assert.equal(result.code, '422_REQUIRED_IMAGE_SLOT_MISSING');
});

test('validateSlide fails when image requirement is malformed', () => {
  const slide = {
    role: 'problem',
    templateId: 'template_explainer_01',
    textAssignments: {
    ...
    imageRequirements: {
      hero: { kind: 'brief', prompt: '   ' }
    }
  };

  const result = validateSlide(workspace, slide);
  assert.equal(result.ok, false);
  assert.equal(result.code, '422_IMAGE_REQUIREMENT_INVALID');
});
